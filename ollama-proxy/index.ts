...
const defaultModel = process.env.DEFAULT_MODEL ?? "gemma3:4b";

function authorized(request: express.Request): boolean {
  return !authToken || request.header("authorization") === `Bearer ${authToken}`;
}
function rejectUnauthorized(response: express.Response): express.Response {
  return response.status(401).json({ error: { code: "UNAUTHORIZED", message: "A valid bearer token is required." } });
}
...
app.post("/caption", express.raw({ type: () => true, limit: "10mb" }), async (request, response) => {
  ...
  try {
    payload = parseMultipart(request.body, request.header("content-type"));
  } catch {
    return response
      .status(400)
      .json({ error: { code: "INVALID_IMAGE", message: "Unable to parse multipart/form-data." } });
  }
  if (!payload.image?.length)
    return response.status(400).json({ error: { code: "INVALID_IMAGE", message: "The image field is required." } });
  ...
  try {
    const upstream = await fetch(`${ollamaUrl}/api/chat`, {
      ...
      body: JSON.stringify({
        model,
        messages: [{ role: "user", content: payload.fields.prompt ?? "", images: [payload.image.toString("base64")] }],
        stream: false,
      }),
    });
    ...
    if (!upstream.ok) {
      ...
      return response
        ...
        .json({ error: { code: unavailable ? "UNSUPPORTED_MODEL" : "MODEL_UNAVAILABLE", message: detail } });
    }
    if (typeof result.message?.content !== "string" || !result.message.content.trim())
      return response
        .status(422)
        .json({ error: { code: "CAPTION_UNAVAILABLE", message: "Ollama did not return a description." } });
    ...
    return response.json({
      ...
      confidenceReasons: reliability.confidenceReasons,
      model,
    });
  } catch {

...
app.post("/caption", express.raw({ type: () => true, limit: "10mb" }), (request, response) => {
  ...
  if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) return response.status(400).json({ error: { code: "INVALID_MOCK_CONFIDENCE", message: "X-Mock-Confidence must be a number from 0 to 1." } });
  ...
  return response.json({ caption: "This is an image description returned by the mock server.", confidence, model: fields.model?.trim() || "altbridge-mock-1" });
});

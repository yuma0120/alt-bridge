...
export interface CaptionRequest { image: Blob | ArrayBuffer; prompt?: string; maxSize?: number; }
...
export interface Settings { endpoint: string; prompt: string; maxSize: number; lowConfidenceThreshold: number; highConfidenceThreshold: number; }
export interface CachedCaption extends CaptionResponse { createdAt: number; }
export type ExtensionMessage = { type: "getImages" } | { type: "captionUrl"; src: string } | { type: "health" };

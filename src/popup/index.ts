...
async function start(): Promise<void> {
  ...
  render(data.images.filter((image) => image.category !== "excluded"), health.available, settings, locale);
}
...
function render(images: ImageRecord[], available: boolean, settings: Settings, locale: SupportedLocale): void {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-src]").forEach((button) => button.onclick = async () => {
    button.disabled = true; button.textContent = t(locale, "generating");
    ...
    if (result.error) {
      ...
      const source = result.confidenceSource === "heuristic" ? t(locale, "heuristicEstimate") : t(locale, "providerConfidence");
    ...
    button.textContent = t(locale, "generateAgain"); button.disabled = false;
  });
  if (!available) document.querySelectorAll<HTMLButtonElement>("[data-src]").forEach((button) => button.disabled = true);
}

function card(image: ImageRecord, locale: SupportedLocale): string { return `<article><img src="${escape(image.src)}" alt="" /><div><strong>${label(image.category, locale)}</strong><p>${escape(image.reason)}</p><p class=alt>alt: ${escape(image.alt ?? t(locale, "missingAltValue"))}</p><button data-src="${escapeAttr(image.src)}">${t(locale, "generate")}</button><p class=result aria-live="polite"></p></div></article>`; }
function label(category: ImageCategory, locale: SupportedLocale): string { const key: Record<ImageCategory, "missingAlt" | "emptyAlt" | "suspiciousAlt" | "validAlt" | "excluded"> = { "missing-alt": "missingAlt", "empty-alt": "emptyAlt", "suspicious-alt": "suspiciousAlt", "valid-alt": "validAlt", excluded: "excluded" }; return t(locale, key[category]); }
...
void start().catch(async (error: unknown) => { const locale = resolveLocale((await getSettings()).language); renderError(error instanceof Error ? error.message : t(locale, "pageUnavailable")); });

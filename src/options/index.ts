...
async function start(): Promise<void> { render(await getSettings()); }
function render(settings: Settings): void {
  ...
  endpoint.oninput = refreshLan; refreshLan();
  language.onchange = () => { const next = resolveLocale(language.value as LanguagePreference); if (settings.promptMode === "preset" || prompt.value === promptFor(locale)) prompt.value = promptFor(next); };
  app.querySelector<HTMLButtonElement>("#restore-prompt")!.onclick = () => { prompt.value = promptFor(resolveLocale(language.value as LanguagePreference)); };
  form.onsubmit = async (event) => {
    event.preventDefault(); const values = Object.fromEntries(new FormData(form)) as Record<string, string>; const normalizedEndpoint = values.endpoint.replace(/\/$/, ""); const preference = values.language as LanguagePreference; const preset = promptFor(resolveLocale(preference));
    const value: Settings = { endpoint: normalizedEndpoint, model: values.model.trim(), authToken: values.authToken.trim(), lanConsent: (form.elements.namedItem("lanConsent") as HTMLInputElement).checked, language: preference, promptMode: values.prompt.trim() === preset ? "preset" : "custom", prompt: values.prompt.trim(), maxSize: Number(values.maxSize), lowConfidenceThreshold: Number(values.lowConfidenceThreshold), highConfidenceThreshold: Number(values.highConfidenceThreshold) };
    if (!valid(value)) return status(t(locale, "invalidSettings"));
    if (!isLoopbackEndpoint(value.endpoint) && (!value.lanConsent || !value.authToken)) return status(t(locale, "invalidLan"));
    await saveSettings(value); render(await getSettings()); status(t(resolveLocale(value.language), "saved"));
  };
  app.querySelector<HTMLButtonElement>("#restore-defaults")!.onclick = () => render({ ...DEFAULT_SETTINGS, prompt: promptFor(resolveLocale(DEFAULT_SETTINGS.language)) });
}
